...
public class SelectElementClassUseCaseImpl implements SelectElementClassUseCase {
    ...
    public void execute(SelectElementClassRequest request) {
        log.info("UC: select element class '" + request.elementClass + "'...");
        ...
        try {
            ...
            var elements = elementService.readElements(request.elementClass, selectDenominations, "", 100); // TODO
            ...
            var message = "successfully read " + denominations.size() + " denominations,  " + elements.size() + " elements "
                + versions.size()  + " versions, " + fwdDependencies.size() + " dependencies and 1 version aggregate";
            log.info(message);
